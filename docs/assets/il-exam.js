    ...
    function render() {
      if (state.finished) {
        ...
        if (!miss.length) {
          ...
          html += '<p class="mt-4 text-sm text-muted">Misses remediate into Adapt + Lab Muse — not an answer dump.</p><ul class="il-exam-remediate mt-6">';
          miss.forEach(function (q) {
            html += '<li class="il-exam-miss"><p class="font-mono text-[0.58rem] uppercase tracking-[0.16em] text-gold">' + esc(q.domain) + '</p><p class="mt-1 text-sm text-paper">' + esc(q.stem) + '</p><p class="mt-2 text-xs text-muted">' + esc(q.why) + '</p><div class="mt-3 flex flex-wrap gap-2"><a class="text-cyan text-xs uppercase tracking-[0.14em]" href="' + q.remediate.adapt + '">Adapt remediations →</a><a class="text-cyan text-xs uppercase tracking-[0.14em]" href="' + q.remediate.coach + '">Ask Muse →</a></div></li>';
          });
          html += '</ul><div class="mt-6 flex flex-wrap gap-3"><button type="button" class="inline-flex h-10 items-center rounded-lg bg-paper px-4 font-display text-[0.65rem] uppercase tracking-[0.14em] text-void" data-il-exam-retry>Retry exam</button><a class="inline-flex h-10 items-center rounded-lg border border-cyan/40 px-4 font-display text-[0.65rem] uppercase tracking-[0.14em] text-cyan" href="/adapt/">Open Adapt</a><a class="inline-flex h-10 items-center rounded-lg border border-paper/15 px-4 font-display text-[0.65rem] uppercase tracking-[0.14em] text-paper" href="/coach/">Lab Muse</a></div>';
        }
