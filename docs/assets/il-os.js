/**
 * Time-to-hire OS — Adapt → Path → Lab → Muse → Evidence export
 * Mount: [data-il-os]
 */
(function (g) {
  'use strict';
  var STAGES = [
    ...
    { id: 'muse', n: '04', title: 'Muse drill', href: '/coach/', body: 'Socratic handoff. Local rules → Ollama when demand grows.',
  ...
  function paint(root) {
    ...
    var html = '<div class="il-os"><p class="il-kicker">Time-to-hire OS · one URL</p><h2 class="mt-2 font-display text-3xl tracking-[-0.03em]">Place → Path → Lab → Muse → Proof</h2><p class="mt-3 max-w-2xl text-sm text-muted">Career-changer loop in five honest stages. Progress is <code class="text-cyan">localStorage</code> you can inspect — never a fake completion certificate.</p>';
    ...
    html += '</ol><div class="il-os-pathpick mt-10"><p class="il-kicker">Path commitment</p><p class="mt-2 text-sm text-muted">Mark one track so the OS can route lab + Muse prompts.</p><div class="il-os-pathgrid mt-4">';
